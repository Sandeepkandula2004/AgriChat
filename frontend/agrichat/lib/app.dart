...
class ChatState extends ChangeNotifier {
  ...
  Future<String?> transcribeAudio(File audioFile) async {
    ...
    try {
      var request = http.MultipartRequest(
        'POST',
        Uri.parse('${BACKEND_BASE}transcribe'),
      );
      ...
      if (resp.statusCode == 200) {
        ...
        debugPrint('Transcription failed: ${resp.statusCode} ${resp.body}');
        return null;
      }
    } catch (e) {
  ...
  Future<void> sendMessage({
    ...
    if (message != null || imageFile != null) {
      String displayMessage = message ?? '[Image uploaded]';
      messages.add({
        'sender': 'user',
    ...
    try {
      ...
      if (isVoiceMessage) {
        request.fields['is_voice_message'] = 'true';
      }
      ...
      if (resp.statusCode == 200) {
        ...
        final audioReply = data['audio_reply'];
        ...
        messages.add({
          ...
          'message': botReply,
          'timestamp': DateTime.now().toIso8601String(),
          'audio_reply': audioReply,
        });

        if (isVoiceMessage && audioReply != null) {
          await playAudioFromBase64(audioReply);
        }
      } else {
...
class _ChatHomePageState extends State<ChatHomePage> {
  ...
  Future<void> _initRecorder() async {
    ...
    if (status != PermissionStatus.granted) {
      ...
      _recorderInitialized = false; // Explicitly set to false
      return;
    }
    
    _recorderInitialized = true; // If permission is granted, we're ready
    debugPrint('Recorder permissions granted');
  }
  ...
  void dispose() {
    ...
    if (_recorderInitialized) {
      _audioRecorder.dispose(); // Use dispose()
    }
    super.dispose();
  }
  ...
  Future<void> _startRecording() async {
    if (!_recorderInitialized) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Recorder not initialized. Please grant microphone permission.')),
      );
      return;
    }
    ...
    try {
      ...
      // The 'record' package uses RecordConfig and an AudioEncoder
    ...
    try {
      ...
      if (path != null) {
        ...
        if (await audioFile.exists()) {
          ...
          final transcribedText = await state.transcribeAudio(audioFile);
          
          ScaffoldMessenger.of(context).hideCurrentSnackBar();
          
          if (transcribedText != null && transcribedText.isNotEmpty) {
            setState(() {
              _inputCtrl.text = transcribedText;
              _isVoiceInput = true;
            });
            
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Audio transcribed! You can edit or send the message.'),
  ...
  Widget _buildMessages(ChatState state) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, i) {
        ...
        final hasImage = m['has_image'] == true;
        final hasAudio = m['audio_reply'] != null;
                  ...
                  children: [
                        ...
                        children: [
                          if (hasImage)
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: isUser ? Colors.white.withOpacity(0.2) : const Color(0xFF2E7D32).withOpacity(0.1),
                                borderRadius: BorderRadius.circular(12),
                              ),
                              child: Row(
                                mainAxisSize: MainAxisSize.min,
                    ...
                    if (hasAudio && !isUser) ...[
                      ...
                      Container(
                        ...
                        child: IconButton(
                          icon: Icon(
                            state.isPlayingAudio ? Icons.stop : Icons.volume_up,
                            color: Colors.white,
                          ),
                          onPressed: () {
                            if (!state.isPlayingAudio) {
                              state.playAudioFromBase64(m['audio_reply']);
                            }
                          },
                        ),
                      ),
        ...
        child: Row(
          children: [
            Container(
              ...
              child: IconButton(
                icon: Icon(
                  _selectedImage != null ? Icons.image : Icons.add_photo_alternate,
                  color: _selectedImage != null ? Colors.amber : Colors.white,
                ),
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: _selectedImage != null 
                        ...
                        : 'Ask about agriculture, farming, crops...',
